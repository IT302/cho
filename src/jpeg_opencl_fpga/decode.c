...
__constant int zigzag_index[64] =	/* Is zig-zag map for matrix -> scan array */
{ 0, 1, 5, 6, 14, 15, 27, 28,
...
WriteBlock (int *store, int *p_out_vpos, int *p_out_hpos,
        unsigned char *p_out_buf, dvars *d)
{
  int voffs, hoffs;
 ...
 */
void
YuvToRgb (int p, int *y_buf, int *u_buf, int *v_buf, dvars *d)
{
  int r, g, b;
  int y, u, v;
  int i;
 ...
 */
void
decode_block (int comp_no, int *out_buf, int *HuffBuff, dvars *d, hvars * h, mvars *m)
{
  int QuantBuff[DCTSIZE2];
  unsigned int *p_quant_tbl;

  DecodeHuffMCU (HuffBuff, comp_no, d, h, m);

  IZigzagMatrix (HuffBuff, QuantBuff);

  p_quant_tbl =
    &(d->p_jinfo_quant_tbl_quantval[(int)d->p_jinfo_comps_info_quant_tbl_no[comp_no]][DCTSIZE2]);
...
decode_start (int *out_data_image_width, int *out_data_image_height,
          int *out_data_comp_vpos, int *out_data_comp_hpos, bvars *b,
          dvars *d, hvars * h, mvars *m)
{
  int i;
  int CurrentMCU = 0;
  ...
  /* Read buffer */
  h->CurHuffReadBufPtr = d->p_jinfo_jpeg_data;
  h->CurHuffReadBuf = m->ReadBuf;
   ...
   */
  *out_data_image_width = d->p_jinfo_image_width;
  *out_data_image_height = d->p_jinfo_image_height;
  ...
  if (d->p_jinfo_smp_fact == SF1_1_1)
    {
       ...
       * 1_1_1
       */
      while (CurrentMCU < d->p_jinfo_NumMCU)
	{

	  for (i = 0; i < NUM_COMPONENT; i++)
	    {
	      decode_block (i, IDCTBuff[i], HuffBuff[i], d, h, m);
	    }


	  YuvToRgb (0, IDCTBuff[0], IDCTBuff[1], IDCTBuff[2], d);
	  /*
	   * Write
	   */
	  for (i = 0; i < RGB_NUM; i++)
	    {
	      WriteBlock (&(d->rgb_buf[0][i][0]),
			  &out_data_comp_vpos[i],
			  &out_data_comp_hpos[i], &(b->OutData_comp_buf[i][0]), d);
	    }
	  CurrentMCU++;
	}
      ...
      while (CurrentMCU < d->p_jinfo_NumMCU)
	{
	  ...
	  for (i = 0; i < 4; i++)
	    {
	      decode_block (0, IDCTBuff[i], HuffBuff[0], d, h , m);
	    }

	  /* Decode U */
	  decode_block (1, IDCTBuff[4], HuffBuff[1], d, h , m);

	  /* Decode V */
	  decode_block (2, IDCTBuff[5], HuffBuff[2], d, h , m);
	  ...
	  for (i = 0; i < 4; i++)
	    {
	      YuvToRgb (i, IDCTBuff[i], IDCTBuff[4], IDCTBuff[5],d);
	    }


	  for (i = 0; i < RGB_NUM; i++)
	    {
	      Write4Blocks (&(d->rgb_buf[0][i][0]),
			    &(d->rgb_buf[1][i][0]),
			    &(d->rgb_buf[2][i][0]),
			    &(d->rgb_buf[3][i][0]),
			    &out_data_comp_vpos[i],
