...
//int i_marker = 0;
__constant int out_unread_marker[READ_MARKERS_REP] =
...
__constant int out_index_get_sof[GET_SOF_REP] = { 0, 1, 2 };
__constant int out_id_get_sof[GET_SOF_REP] = { 1, 2, 3 };
__constant int out_h_samp_factor_get_sof[GET_SOF_REP] = { 2, 1, 1 };
__constant int out_v_samp_factor_get_sof[GET_SOF_REP] = { 2, 1, 1 };
__constant int out_quant_tbl_no_get_sof[GET_SOF_REP] = { 0, 1, 1 };
...
__constant int out_comp_id_get_sos[GET_SOS_REP] = { 1, 2, 3 };
__constant int out_dc_tbl_no_get_sos[GET_SOS_REP] = { 0, 1, 1 };
__constant int out_ac_tbl_no_get_sos[GET_SOS_REP] = { 0, 1, 1 };
...
//int i_get_dht = 0;
__constant int out_length_get_dht[GET_DHT_REP] = { 29, 179, 29, 179 };
__constant int out_index_get_dht[GET_DHT_REP] = { 0x0, 0x10, 0x1, 0x11 };
__constant int out_count_get_dht[GET_DHT_REP] = { 12, 162, 12, 162 };
...
//int i_get_dqt = 0;
__constant int out_length_get_dqt[GET_DQT_REP] = { 65, 65 };
__constant int out_prec_get_dht[GET_DQT_REP] = { 0, 0 };
__constant int out_num_get_dht[GET_DQT_REP] = { 0, 1 };


/*
 * Initialize Quantization Table
 */
__constant int izigzag_index[64] = { 0, 1, 8, 16, 9, 2, 3, 10,
...
read_word (mvars *m)
{
  ...
  c = m->ReadBuf[m->ReadBufPtr++] << 8;
  c |= m->ReadBuf[m->ReadBufPtr++];

  return c;
}

int
first_marker (mvars *m)
{
 ...
 */
void
get_sof (mvars *m, dvars *d,  unsigned char *ReadBuf)
{
  int ci, c;
  int length;
  ...
  length = read_word (m);
  d->p_jinfo_data_precision = read_byte (m);
  d->p_jinfo_image_height = read_word (m);
  d->p_jinfo_image_width = read_word (m);
  d->p_jinfo_num_components = read_byte (m);

  //printf ("length         = %d\n", length);
  //printf ("data_precision = %d\n", d->p_jinfo_data_precision);
  //printf ("image_height   = %d\n", d->p_jinfo_image_height);
  //printf ("image_width    = %d\n", d->p_jinfo_image_width);
  //printf ("num_components = %d\n", d->p_jinfo_num_components);

  if (length != m->out_length_get_sof)
    {
        m->main_result++;
    }
  if (d->p_jinfo_data_precision != m->out_data_precision_get_sof)
    {
        m->main_result++;
    }
  if (d->p_jinfo_image_height != m->out_p_jinfo_image_height_get_sof)
    {
        m->main_result++;
    }
  if (d->p_jinfo_image_width != m->out_p_jinfo_image_width_get_sof)
    {
        m->main_result++;
    }
  if (d->p_jinfo_num_components != m->out_p_jinfo_num_components_get_sof)
    {
        m->main_result++;
    }
  ...
  /* Check components */
  for (ci = 0; ci < d->p_jinfo_num_components; ci++)
    {
      p_comp_info_index = &(d->p_jinfo_comps_info_index[ci]);
      p_comp_info_id = &(d->p_jinfo_comps_info_id[ci]);
      p_comp_info_h_samp_factor = &(d->p_jinfo_comps_info_h_samp_factor[ci]);
      p_comp_info_v_samp_factor = &(d->p_jinfo_comps_info_v_samp_factor[ci]);
      p_comp_info_quant_tbl_no =  &(d->p_jinfo_comps_info_quant_tbl_no[ci]);

      *p_comp_info_index = ci;
      *p_comp_info_id = read_byte (m);
      c = read_byte (m);
      *p_comp_info_h_samp_factor = (c >> 4) & 15;
      *p_comp_info_v_samp_factor = (c) & 15;
      *p_comp_info_quant_tbl_no = read_byte (m);
   ...
   */
  if (d->p_jinfo_comps_info_h_samp_factor[0] == 2)
    {
      d->p_jinfo_smp_fact = SF4_1_1;
      //printf ("\nSampling Factor is 4:1:1\n");
    }
  else
    {
      d->p_jinfo_smp_fact = SF1_1_1;
...
void
get_sos (mvars *m, dvars *d,  unsigned char *ReadBuf)
{
  int length, num_comp;
  int i, c, cc, ci, j;
  char *p_comp_info_id;
  ...
  length = read_word (m);
  num_comp = read_byte (m);

  //printf (" length = %d\n", length);
  //printf (" num_comp = %d\n", num_comp);

  if (length != m->out_length_get_sos)
    {
        m->main_result++;
    }
  if (num_comp != m->out_num_comp_get_sos)
    {
        m->main_result++;
    }
  ...
  {
    p_comp_info_id = &(d->p_jinfo_comps_info_id[ci]);
    p_comp_info_dc_tbl_no = &(d->p_jinfo_comps_info_dc_tbl_no[ci]);
    p_comp_info_ac_tbl_no = &(d->p_jinfo_comps_info_ac_tbl_no[ci]);
    ...
    id_found:
     ...
     // printf (" ac_tbl_no     = %d\n", *p_comp_info_ac_tbl_no);
      ...
      if (cc != out_comp_id_get_sos[m->i_get_sos])
  {
    m->main_result++;
  }
      if (*p_comp_info_dc_tbl_no != out_dc_tbl_no_get_sos[m->i_get_sos])
  {
    m->main_result++;
  }
      if (*p_comp_info_ac_tbl_no != out_ac_tbl_no_get_sos[m->i_get_sos])
  {
    m->main_result++;
  }
      (m->i_get_sos)++;
   ...
   */
  d->p_jinfo_jpeg_data = m->ReadBufPtr;
 ...
 */
void
get_dht (mvars *m, dvars *d,  unsigned char *ReadBuf)
{
  int length;
  int index, i, count;
  int *p_xhtbl_bits;
  ...
  if (length != out_length_get_dht[m->i_get_dht])
    {
        m->main_result++;
    }
  ...
  while (length > 16)
    {
      ...
      if (index != out_index_get_dht[m->i_get_dht])
  {
        m->main_result++;
}
      ...
      if (index & 0x10)
  {
    /* AC */
    index -= 0x10;
    p_xhtbl_bits = d->p_jinfo_ac_xhuff_tbl_bits[index];
    p_xhtbl_huffval = d->p_jinfo_ac_xhuff_tbl_huffval[index];
  }
      else
  {
    /* DC */
    p_xhtbl_bits = d-> p_jinfo_dc_xhuff_tbl_bits[index];
    p_xhtbl_huffval = d->p_jinfo_dc_xhuff_tbl_huffval[index];
  }
      ...
      for (i = 1; i <= 16; i++)
  {
    p_xhtbl_bits[i] = read_byte (m);
    count += p_xhtbl_bits[i];
  }

     // printf (" count = %d\n", count);

      if (count != out_count_get_dht[m->i_get_dht])
  {
        m->main_result++;
    }
      m->i_get_dht++;
...
void
get_dqt (mvars *m, dvars *d,  unsigned char *ReadBuf)
{
  int length;
  int prec, num, i;
  unsigned int tmp;
  ...
  if (length != out_length_get_dqt[m->i_get_dqt])
    {
        m->main_result++;
    }
  ...
  while (length > 0)
    {
      ...
      if (prec != out_prec_get_dht[m->i_get_dqt])
  {
        m->main_result++;
    }
      if (num != out_num_get_dht[m->i_get_dqt])
  {
        m->main_result++;
    }
      m->i_get_dqt++;

      p_quant_tbl = &(d->p_jinfo_quant_tbl_quantval[num][DCTSIZE2]);
...
void
read_markers (mvars *m, dvars *d,  unsigned char *ReadBuf, unsigned char *buf)
{
	int unread_marker;
	int sow_SOI;
	...
	for (;;) {
		if (!sow_SOI) {
			//printf (" Lake Tahoe !\n");
			unread_marker = first_marker(m);
		} else {
			//printf (" Lake Tahoe2 !\n");
			unread_marker = next_marker(m);
		}

		//printf("\nmarker = 0x%x\n", unread_marker);

		if (unread_marker != out_unread_marker[m->i_marker++]) {
			m->main_result++;
		}
