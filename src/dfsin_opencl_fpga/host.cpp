...
int main(int argc, char* argv[])
{
       ...
       if (status != CL_SUCCESS)
   {
    ...
    auto  num  = N;
    std::vector<cl_ulong, AAlloc::AlignedAllocator<cl_ulong, 64>> Keys_in;
    std::vector<cl_ulong, AAlloc::AlignedAllocator<cl_ulong, 64>> Keys_out;
   ...
   if (status != CL_SUCCESS)
   {
    ...
    cl_kernel kernel = clCreateKernel(program,"dfsin_main", &status);
    if (status != CL_SUCCESS)
    {
    ...
    std::cout<<"Lunching dfsin kernel!"<<std::endl;
...
/*    for (cl_int i : Keys_out )
    ...
    for (int i  = 0; i < 8; i++)
    {
        ...
        {
            std::cout<<"dfsin failed"<<std::endl;
