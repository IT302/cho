     ...
     globvars *g;
{
  ...
  motion_code = Get_motion_code (g);

  motion_residual = (h_r_size != 0
		     && motion_code != 0) ? Get_Bits (h_r_size, g) : 0;

  decode_motion_vector (&PMV[0], h_r_size, motion_code, motion_residual,
			full_pel_vector);

  if (dmv)
    dmvector[0] = Get_dmvector (g);


  /* vertical component */
  motion_code = Get_motion_code (g);
  motion_residual = (v_r_size != 0
		     && motion_code != 0) ? Get_Bits (v_r_size, g) : 0;
...
{ 
      ...
      if (mv_format == MV_FIELD && !dmv)
  {
    motion_vertical_field_select[1][s] =
      motion_vertical_field_select[0][s] = Get_Bits (1, g);
  }
  ...
  else
    {
      ...
      motion_vertical_field_select[1][s] = Get_Bits (1, g);
